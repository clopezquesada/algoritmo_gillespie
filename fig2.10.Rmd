...
\[
  \emptyset\rightarrow X, \quad a_{\text{nacimiento}}=k_1
\]
\[
  X\rightarrow \emptyset, \quad a_{\text{muerte}}=k_2X
\]

```{r}
NacimientoMuerte_Gillespie <- function(k_1,k_2,x0,t0,tf){
...
for (i in 1:n_realizaciones) {
  realizacion <- NacimientoMuerte_Gillespie(k1,k2,x0,t0,tf)
...
sol_ec_maestra <- function(k1,k2,t,x){
  lambda <- k1/k2*(1-exp(-k2*t))
...
grafico <- ggplot(data = datos, aes(x=datos, color="Dist. Número de Individuos")) + 
      geom_histogram(aes(y = ..density..), colour = 1, fill = "white") + geom_density() + 
