...
for (i in 1:n_realizaciones) {
  ...
  if(i<=10){
    ...
    grafico <- grafico + 
      geom_line(data = imprimir, aes(x = t_muestreo/60, y = x_muestreo, color="Realizaciones"))
  }
}
...
grafico <- grafico + geom_line(lwd=0.7,data=datos_est,aes(x = t_muestreo/60, y = media,color="Media"))
grafico <- grafico + geom_line(lwd=0.7,data=datos_est,aes(x = t_muestreo/60, y = media-desvtipica,color="Intervalo de confianza"))
grafico <- grafico + geom_line(lwd=0.7,data=datos_est,aes(x = t_muestreo/60, y = media+desvtipica,color="Intervalo de confianza"))
grafico <- grafico + geom_line(lwd=0.7,data=out,aes(x = time/60, y = X,color="Sol. mod. det."))

grafico <- grafico +
   labs(x = "Tiempo (min)", y = " Número de individuos") +
   theme_minimal() +
