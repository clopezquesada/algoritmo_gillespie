...
Resolvemos la ecuación en derivadas parciales asociada al modelo para más tarde poder representarla gráficamente y compararla con las realizaciones:
\[
...
u0 <- function(x){
  x0 <- rep(0, length(x))
  x0[1] <- 30
  #x0 <- (1/(sqrt(2*pi*0.2^2)))*exp(-(x^2)/(2*0.2^2))
...
```{r}
FKPP <- function(L,D,K,r,t0,tf,N,t_muestreo){
  t <- t0
...
df <- FKPP(L=L,D=D,K=K,r=r,t0=t0,tf=tf,N=N,t_muestreo=t_muestreo)
...
for (i in 1:n_realizaciones) {
  df_aux <- FKPP(L=L,D=D,K=K,t0=t0,tf=tf,N=N,t_muestreo=t_muestreo)
