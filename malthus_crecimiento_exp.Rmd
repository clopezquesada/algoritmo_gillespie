...
Malthus<-function(r,x0,t_muestreo){
    ...
    for (j in 1:length(t_muestreo)) { # Para cada tiempo de muestreo, si hemos alcanzado un tiempo de reacción, calculamos la reacción que ocurre
...
# Definimos el gráfico p donde iremos superponiendo los resultados gráficos de las distintas realizaciones
...
# Añadimos título, subtítulo y nombre a los ejes, fijamos el eje y para poder observar que a mayor número de realizaciones más se acerca el valor de la media a la solución de la ecuación diferencial del modelo
grafico <- grafico +
  ylim(0,50) +
  labs(title = "Modelo Malthus", subtitle = paste(n_realizaciones, " realizaciones", sep=" "), x = "Tiempo (s)", y = " Nº de Individuos en la población") + theme(legend.title=element_blank(),legend.position = "bottom") +
      scale_color_manual(values = c("Solución" = "red", "Realizaciones"="grey", "Media"="blue", "Delta"="green" ))

# Ponemos la función suppressWarnings() porque al fijar el eje y, al imprimir el gráfico da un error como que hay datos que se pierden
